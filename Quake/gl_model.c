...
//johnfitz
	...
	for (i=0 ; i<nummiptex ; i++)
	{
		...
		if (pal)
		{
			palette_p = pixels_p + pixels / 64 * 85;
		...
		if ((pixels_p + lump_bytes) > (mod_base + l->fileofs + l->filelen))
		{
			Con_DPrintf("Texture %s extends past end of lump\n", mt.name);
			lump_bytes = q_max(0L, (long)((mod_base + l->fileofs + l->filelen) - pixels_p));
#ifdef BSP29_VALVE
			if (pal)
				palette = q_min(palette, q_max(0, lump_bytes - pixels / 64 * 85));
#endif
...
static void Mod_ProcessLeafs_S (byte *in, int filelen)
{
	mleaf_t		*out;
	int			i, j, count, p;
...
static void Mod_ProcessLeafs_L1 (byte *in, int filelen)
{
	mleaf_t		*out;
	int			i, j, count, p;
...
static void Mod_ProcessLeafs_L2 (byte *in, int filelen)
{
	mleaf_t		*out;
	int			i, j, count, p;
...
===============
*/
static void *Mod_LoadAllSkins (int numskins, byte *pskintype)
{
